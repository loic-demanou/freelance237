<div>
    <div class="max-w-1/2 w-1/2 mx-auto">
        <h1 class="text-3xl text-indigo-500 mb-4">Mission : {{ $conversation->job ?  $conversation->job->title  : "Direct conversation" }}</h1>
        @foreach($conversation->messages as $message)
            <div class="rounded-lg px-3 py-1 mb-2 font-medium
            {{ $message->user->id === auth()->user()->id  ? 'bg-indigo-500 text-white mr-auto max-w-1/2 w-1/2' : 'ml-auto bg-gray-300 text-gray-700 max-w-1/2 w-1/2'}}">
