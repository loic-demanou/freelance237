    ...
    <div class="col-md-6 col-sm-12 col-lg-12">
        ...
        <div class="px-3 py-3 mb-5 hover:shadow-lg rounded border border-gray-500 bg-gray-200">
            <span class="text-sm text-gray-600 d-flex mb-4"><strong class="mr-1"> {{" ". $job->user->name }} </strong>
                <img src="{{ asset('storage') . '/' .$job->user->profile_photo_path }}" width="20px" height="5px"
            ...
            @endif
            {{-- <a style=" font-weight:bold" href="{{ route('jobs.show', $job->id) }}">Show the mission
                </a> --}}
...
<div class="text-sm text-gray-600 mt-2 ">There is {{ $job->proposals->count() }} @choice('proposal|proposals',
    $job->proposals) for this mission</div>
{{-- <a href="{{ route('jobs.show', $job->id) }}">Show</a> --}}

{{-- <span class="text-sm text-gray-600 mt-2 d-flex fa-pull-right"><a
        href="{{ route('jobs.show', $job->id) }}">Show</a></span> --}}
