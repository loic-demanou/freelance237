<a  href="{{ route('jobs.show', $job->id) }}" class=" 
    inline-block text-white  hover:text-green-500 duration-200">
<div class="col-md-6 col-sm-12 col-lg-12">
    <div class="px-3 py-3 mb-5 hover:shadow-lg rounded border border-gray-500 bg-gray-200">
            <span class="text-sm text-gray-600 d-flex mb-4"><strong class="mr-1"> {{" ". $job->user->name }} </strong>
                <img src="{{ asset('storage') . '/' .$job->user->profile_photo_path }}"  width="20px" height="5px" class="rounded-circle mr-2 ">
                {{ $job->created_at->diffForHumans() }}
            </span>
                    <div class="flex justify-between">

            <h2 class="text-xl font-bold text-green-600 mb-2">{{ $job->title }}</h2>
        ...
        <div class="flex items-center">
            ...
            @else
                <span class="h-3 w-3 bg-red-500 rounded-full mr-3"></span><p class="mr-4 text-gray-600"">Unavailable </p>
            @endif
        ...
        {{-- <span class="text-sm text-gray-600 mt-2 d-flex fa-pull-right">Published by | <strong class="mr-1"> {{" ". $job->user->name }} </strong>
        <img src="{{ asset('storage') . '/' .$job->user->profile_photo_path }}"  width="20px" height="5px" class="rounded-circle mr-2 ">
        {{ $job->created_at->diffForHumans() }}
        </span> --}}
